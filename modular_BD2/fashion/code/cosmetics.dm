...
/obj/item/cosmetics/mirror_makeup/attack_self(mob/living/carbon/human/user)
	...
	SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "mirrorcheck", /datum/mood_event/mirrorcheck)
	...
	if(ishuman(M))
		...
		if(H == user)
			...
			if(do_after(user, 50, target = H))
				SEND_SIGNAL(user, COMSIG_ADD_MOOD_EVENT, "makeup", /datum/mood_event/makeup)
				H.lip_style = "makeup"
				H.lip_color = null
				H.update_body()
		else
...
/datum/mood_event/mirrorcheck
	description = span_nicegreen("I liked looking in the mirror.")
	mood_change = 1
	timeout = 5 MINUTES

/datum/mood_event/makeup
	description = span_nicegreen("I feel pretty.")
	mood_change = 1
	timeout = 15 MINUTES
